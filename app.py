...
progress = {
    ...
    "filename": "",
    "log": [],
    "files": []               # لیست فایل‌های تمام‌شده برای دانلود کاربر
}
...
def build_ydl_opts(quality: str, ffmpeg_location: str | None = None):
    """
    منطق کیفیت دقیقاً مثل نسخه اصلی شما:
    ...
    ydl_opts = {
        ...
        "quiet": True,
        "noprogress": True,
        # کلاینت‌های مقاوم‌تر در برابر تشخیص ربات روی سرورهای ابری
        "extractor_args": {
            "youtube": {
    ...
    with YoutubeDL(ydl_opts) as ydl:
        for ln in links:
            try:
                ...
                err = str(e)
                # پیام واضح‌تر برای خطای معروف یوتیوب
                if "Sign in to confirm" in err or "not a bot" in err:
                    short = "یوتیوب این IP را بلاک کرده (ربات تشخیص داده)"
                else:
                    short = err[:120]
                results.append({
                    "title": f"خطا: {short}",
    ...
    while True:
        ...
        try:
            ...
            progress.update({
                ...
                "eta": "",
                "filename": "",
                "files": []          # ریست لیست فایل‌ها برای جاب جدید
            })
            ...
            with YoutubeDL(ydl_opts) as ydl:
                for i, ln in enumerate(links, 1):
                    ...
                    try:
                        ...
                        # بعد از دانلود موفق، فایل را پیدا کن و به لیست اضافه کن
                        # چون restrictfilenames=True است، نام فایل تمیز شده است
                        # ما آخرین فایل‌های ساخته‌شده را بررسی می‌کنیم
                        time.sleep(0.3)  # کمی صبر برای اطمینان از نوشته شدن فایل
                        files_in_dir = sorted(
                        ...
                        for f in files_in_dir:
                            if f.is_file() and f.suffix.lower() in {".mp4", ".mkv", ".webm", ".m4a"}:
                                # فقط فایل‌هایی که هنوز در لیست نیستند
...
def start():
    ...
    # پاک کردن فایل‌های قدیمی برای صرفه‌جویی در فضای دیسک (مهم روی Render)
    try:
...
def download_file(filename):
    """دانلود مستقیم فایل به سیستم کاربر"""
    # امنیت ساده: فقط اجازه دانلود از پوشه downloads
...
@app.route("/files")
def list_files():
    """لیست فایل‌های موجود در پوشه downloads"""
    files = []
