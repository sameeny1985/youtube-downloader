...
def analyze_links(raw_links: str):
    ...
    # چند ترکیب کلاینت را امتحان می‌کنیم تا یکی جواب بدهد
    client_sets = [
        ["android"],
        ["android_vr"],
        ["tv_downgraded"],
        ...
        if not success:
            ...
            if "Sign in to confirm" in err or "not a bot" in err:
                if cookies:
                    ...
                    short = "یوتیوب این IP را بلاک کرده — کوکی آپلود کنید"
            elif "Requested format" in err:
                short = "مشکل موقت در خواندن اطلاعات ویدیو — دوباره امتحان کنید"
            else:
                short = err[:140]
            results.append({
                "title": f"خطا: {short}",
